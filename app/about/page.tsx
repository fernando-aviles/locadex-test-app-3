...
export default function About() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24">
      ...
      <h1 className="text-4xl font-bold mb-4">{m(aboutStrings.heading)}</h1>
      <p className="text-lg text-gray-600 mb-10 max-w-md text-center">
        {m(aboutStrings.intro)}
      </p>

      <div className="grid grid-cols-1 gap-8 max-w-lg w-full mb-12">
        <div>
          <h2 className="text-xl font-semibold mb-2">{m(aboutStrings.missionLabel)}</h2>
          <p className="text-gray-600">{m(aboutStrings.missionText)}</p>
        </div>
        <div>
          <h2 className="text-xl font-semibold mb-2">{m(aboutStrings.teamLabel)}</h2>
          <p className="text-gray-600">{m(aboutStrings.teamText)}</p>
        </div>
      </div>

      <div className="max-w-lg w-full">
        <h2 className="text-xl font-semibold mb-4"><T id="about.values.heading">Our Values</T></h2>
        <p className="text-gray-600 mb-6">
          <T id="about.values.description">These are the principles that guide every decision we make as a team.
          They are not just words on a page — they shape how we work day to day.</T>
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {values.map((value) => (
            <div key={m(value.title)} className="border rounded-xl p-4">
